...
statement returns [List<Statement> ast = new ArrayList<Statement>()] locals[List<Expression>args = new ArrayList<Expression>(),
          ...
          // EJERCICIO
          | 'for' '(' v1=varDefinitionAs ';' e1=expression ';' a1=post ')' b1=body
                {$ast.add(new For($v1.ast, $e1.ast, $a1.ast, $b1.ast, $v1.ast.getLine(), $v1.ast.getColumn()));}
          // Return
...
// EJERCICIO: int i = 0
varDefinitionAs returns [VarDefinitionWithAssigment ast]:
        t1=type ID '=' e1=expression
        {
            VarDefinition var = new VarDefinition($ID.text, $t1.ast, $ID.getLine(), $ID.getCharPositionInLine()+1);
            $ast = new VarDefinitionWithAssigment(var, $e1.ast, var.getLine(), var.getColumn());
        }
        ;

// EJERCICIO: i=i+1
//assigmentFor returns [Statement ast]:
//    e1=expression '=' e2=expression {$ast = new Assigment($e1.ast, $e2.ast, $e1.ast.getLine(), $e1.ast.getColumn());}
//    ;

post returns [Post ast]:
    e1=expression op=('++' | '--')
    {$ast = new Post($e1.ast, $op.text, $e1.ast.getLine(), $e1.ast.getColumn());}
    ;
