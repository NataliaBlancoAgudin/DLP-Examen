...
public class ExecuteCGVisitor extends AbstractCGVisitor<Void, FunctionDefinition> {
    ...
    /**
     *     execute[[Post:                  stmt    -> expr op=(++|--)]]()=
     *         address[[expr]]()
     *         value[[expr]]()
     *         cg.convertTo(expr.type, IntType)
     *         <push> 1
     *         cg.post(stmt.op, IntType)
     *         <store> expr.type.suffix
     *
     */
    @Override
    public Void visit(Post p, FunctionDefinition param) {
        p.getExpression().accept(addressCGVisitor, null);
        p.getExpression().accept(valueCGVisitor, null);
        getCodeGenerator().convertTo(p.getExpression().getType(), IntType.getInstance());
        getCodeGenerator().pushi(1);
        getCodeGenerator().post(p.getOperator(), IntType.getInstance());
        getCodeGenerator().store(p.getExpression().getType());
        return null;
    }
}
