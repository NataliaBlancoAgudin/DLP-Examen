...
//    }
     ...
     */
    @Override
    public Void visit(TernaryOperator t, Void param) {
        getCodeGenerator().commentLine(t.getLine());
        getCodeGenerator().comment("TernaryOperator");

        String falso = getCodeGenerator().getLabel();
        String end = getCodeGenerator().getLabel();

        t.getLeft().accept(this, null);
        getCodeGenerator().jz(falso);

        getCodeGenerator().comment("true body");
        t.getRightTrue().accept(this, null);
        getCodeGenerator().convertTo(t.getRightTrue().getType(), t.getType());
        getCodeGenerator().jmp(end);

        getCodeGenerator().comment("false body");
        getCodeGenerator().insertLabel(falso);
        t.getRightFalse().accept(this, null);
        getCodeGenerator().convertTo(t.getRightFalse().getType(), t.getType());
