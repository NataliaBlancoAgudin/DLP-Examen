...
public class ExecuteCGVisitor extends AbstractCGVisitor<Void, FunctionDefinition> {
    ...
    /**
     * execute[[DoWhile: stmt1 -> expr1 stmt2*]]()=
     *         <#line> expr.line
     *         <' * DoWhile>
     *         String doBody = cg.getLabel()
     *
     *         <' * DoWhile body>
     *         do <:>
     *         stmt2*.forEach(s -> execute[[s]]())
     ...
     */
    @Override
    public Void visit(DoWhile d, FunctionDefinition param) {
        getCodeGenerator().commentLine(d.getLine());
        getCodeGenerator().comment("Do while");

        String doBody = getCodeGenerator().getLabel();

        getCodeGenerator().insertLabel(doBody);
        getCodeGenerator().comment("DoWhile body");
        for(Statement st: d.getBody()){
            st.accept(this, null);
        }

        d.getCondition().accept(valueCGVisitor, null);
        getCodeGenerator().convertTo(d.getCondition().getType(), IntType.getInstance());
        getCodeGenerator().jnz(doBody);
